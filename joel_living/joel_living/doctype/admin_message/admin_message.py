...
def add_to_inbox(subject, message, recipients, sent_date=None, sender=None):
    """Insert Admin Message Inbox entries with correct sender full name"""
    inbox_map = {}
 
    # Get full name of sender (who sent the message)
    ...
    for user in recipients:
        doc = frappe.get_doc({
            ...
            "title": subject,
            "content": message,
            "recipient": user,                 # user who receives the message
            "send_date": sent_date or frappe.utils.now_datetime(),
            "sent_by": sender_full_name,       # ✅ Store sender name correctly
        })
...
def send_system_message_job(subject, message_body, sender, recipients, message_docname):
    ...
    try:
        ...
        inbox_map = add_to_inbox(subject, message_body, recipients, sent_date , sender)
        valid_emails = []
