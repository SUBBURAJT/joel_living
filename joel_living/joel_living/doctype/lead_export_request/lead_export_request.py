...
class LeadExportRequest(Document):
    ...
    def after_insert(self):
        """
        If Admin / Super Admin creates request → Auto approve & export.
        Else → Send for approval.
        """
 
        # ================================
        # ✅ NEW LOGIC : CHECK ADMIN USER
        # ================================
        ...
        # ================================
        # ✅ CHANGED : AUTO EXPORT FOR ADMINS
        # ================================
        if is_admin_user:
            ...
            # Directly approve and export
            enqueue(
                "joel_living.joel_living.doctype.lead_export_request.lead_export_request.approve_export_request",
                queue="long",
                job_name=f"auto_export_{self.name}",
                docname=self.name
            )
        else:
            # ORIGINAL FLOW FOR NORMAL USERS
            enqueue(
                self.send_notifications,
                queue='short',
                job_name=f"lead_export_notify_{self.name}"
...
def create_export_request(leads):
    ...
    status = "Approved" if is_admin_user else "Pending Approval"  # ✅ CHANGED
